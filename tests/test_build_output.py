...
class GeneratedOutputTests(unittest.TestCase):
    ...
    def test_home_is_a_plain_document_list(self):
        ...
        self.assertEqual([node.get_text(strip=True) for node in soup.select(".document-release strong")], ["2026-v1"] * 2)
    ...
    def test_reader_has_no_search_and_opens_pdf_in_browser(self):
        ...
        self.assertEqual(soup.select_one(".reader-version").get_text(strip=True), "2026-v1")
        self.assertTrue(soup.title.get_text(strip=True).startswith("2026-v1 "))
