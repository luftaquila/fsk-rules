...
\begin{enumerate}
  \item\label{rule:formula-technical.roll-hoops.common} 메인 롤 후프 및 전방 롤 후프(Main and Front Hoops) 일반 규정
    \begin{enumerate}
      \item\label{rule:formula-technical.roll-hoops.common.helmet-clearance} 가장 키가 큰 드라이버와 일반인 평균의 상위 95\%(운전석 윗면에서 헬멧을 쓴 머리까지의 거리는 1,000mm로 가정)에 해당하는 키의 드라이버가 평상시대로 운전석에 앉아 안전벨트 및 보호장비를 착용했을 때, 메인 후프의 최상부와 전방 롤 후프 최상부에 또는 메인 후프 지지대 하단부의 연장선에서부터 드라이버의 헬멧 사이의 간격이 50mm 이상이어야 한다. (\figref{fig:메인 롤 후프 및 전방 롤 후프 일반 규정} 참고)
        \begin{enumerate}
          \item\label{rule:formula-technical.roll-hoops.common.helmet-clearance.percy-template} 일반인 평균 상위 95\% 드라이버 모델 (\figref{fig:일반인 평균 상위 95퍼센트 드라이버 모델} 참고)
          \begin{enumerate}
      ...
      \figwithcaption{일반인 평균 상위 95퍼센트 드라이버 모델}{일반인 평균 상위 95\% 드라이버 모델}{formula}{0.8}
  ...
  \item\label{rule:formula-technical.roll-hoops.main-roll-hoop} 메인 롤 후프 (Main Roll Hoop) \label{item:메인 롤 후프}
    \begin{enumerate}
      ...
      \item\label{rule:formula-technical.roll-hoops.main-roll-hoop.monocoque-continuous-tube} 모노코크 구조의 경우에도 마찬가지로 메인 롤 후프는 하나의 끊어지지 않은 원형 파이프로 한쪽(왼쪽) 밑바닥에서 드라이버 위를 지나 반대쪽(오른쪽) 바닥면까지 이어져 있어야 한다. \label{item:모노코크 메인 롤 후프}
        \begin{enumerate}
    ...
    \begin{enumerate}
      \item\label{rule:formula-technical.impact-attenuator.structure.material-and-attachment} 재료의 조건과 부착방법
        \begin{enumerate}
          ...
          \item\label{rule:formula-technical.impact-attenuator.structure.material-and-attachment.exempt-if-reference-design} \ref{item:충격완화장치 예시} 1\string~5번을 충족하는 충격완화장치를 제작, 장착하였을 경우에는 \ref{item:충격완화장치}을 증빙하는 데이터 제출은 하지 않아도 된다. (\figref{fig:충격완화장치 예시} 참고)
    ...
    \begin{enumerate}
      ...
      \item\label{rule:formula-technical.side-impact-structure.tube-frame.three-members} 다음에 명시된 3개의 부재는 \ref{section:재료의 최소 요구조건}에 명시된 재료의 조건을 만족해야 한다.
        \begin{enumerate}
          ...
          \item\label{rule:formula-technical.side-impact-structure.tube-frame.three-members.diagonal-member-connectivity} 대각선 부재(Diagonal Member): 상단 부재와 하단 부재를 연결하고 메인 롤 후프와 전방 롤 후프와도 연결되어야 한다. 다수의 부재로 구성이 되었을 경우, 대각선 부재의 한 쪽 끝단은 연결되어 있어 트러스구조를 이루어야 한다.
...
{\color{blue}
\section{운전석 - Drivers seat}\label{rule:formula-technical.cockpit} \label{section:운전석}
    ...
    \begin{enumerate}
      \item\label{rule:formula-technical.fire-extinguishers.extinguishers.types-and-quantities} 각 팀은 화재에 대비하여 소화기를 준비하여야 하고, 검차 시 확인받아야 한다.
        \begin{itemize}
          ...
          \item \string[E-Formula\string]: 리튬배터리용 소화기 (아래 소화기 품명) 최소 1개 및 1kg 이상 분말 ABC 소화기 최소 1개
            \begin{itemize}
...
\begin{enumerate}
  ...
  \item\label{rule:formula-technical.fuel-system.fuel-lines} 연료라인, 라인 장착과 보호
    \begin{enumerate}
      ...
      \figwithcaption{연료라인용 둥근고리 피팅과 가시모양 피팅 및 클램프}{연료라인용 둥근고리 피팅, 가시모양 피팅, 클램프}{formula}{0.8}
  ...
  \item\label{rule:formula-technical.fuel-system.direct-injection} 고압분사(High Pressure Injection, HPI) / 직분사(Direct Injection, DI) 시스템\\
    ...
    \begin{enumerate}
      \item\label{rule:formula-technical.fuel-system.direct-injection.high-pressure-line} 고압 연료라인
        \begin{enumerate}
          \item\label{rule:formula-technical.fuel-system.direct-injection.high-pressure-line.stainless-mesh-hose} 유연성이 있는 스테인레스 스틸(Stainless Steel)재질의 매쉬호스를 사용해야 한다.
  ...
  \figwithcaption{연료 시스템과 엔진 흡기 시스템 위치조건}{연료 시스템, 엔진 흡기 시스템 위치조건}{formula}{0.6}
\end{enumerate}
...
\begin{enumerate}
  \item\label{rule:formula-technical.electronic-throttle-control.principles} 일반
    \begin{enumerate}
      ...
      \item\label{rule:formula-technical.electronic-throttle-control.principles.documented-in-etcsf} ETC 시스템의 기능 등에 관한 것은 \ref{item:ETC 시스템 보고서} ETC 시스템 보고서에 따라 작성 제출하여야 한다.
    \end{enumerate}
  ...
  \item\label{rule:formula-technical.electronic-throttle-control.tps} 스로틀 위치 센서 (Throttle Position Sensor, TPS)
    \begin{enumerate}
      ...
      \item\label{rule:formula-technical.electronic-throttle-control.tps.digital-fault-handling} {\color{blue}어떠한} 종류의 디지털 데이터 전송을 사용하여 TPS 신호를 전송하는 경우{\color{blue}이더라도},
        \begin{enumerate}
          \item\label{rule:formula-technical.electronic-throttle-control.tps.digital-fault-handling.failure-modes-in-etcsf} ETC 시스템 보고서에는 발생할 수 있는 모든 잠재적 고장 모드, 이러한 고장을 탐지하는데 사용되는 방법 및 탐지 방법이 정상적으로 작동하는지 입증하기 위해 수행된 테스트에 대한 자세한 설명이 포함되어야 한다.
  ...
  \item\label{rule:formula-technical.electronic-throttle-control.bspd} 제동시스템 타당성 장치 (Brake System Plausibility Device, BSPD)
    \begin{enumerate}
      \item\label{rule:formula-technical.electronic-throttle-control.bspd.standalone-nonprogrammable-circuit} 전자 스로틀 제어를 모니터링하기 위해서는 반드시 독립형의 프로그래밍이 불가능한 회로를 사용해야 한다. BSPD는 반드시 스로틀 타당성 검사(⑧항)에 포함되어야 한다.
      \item\label{rule:formula-technical.electronic-throttle-control.bspd.direct-sensor-signals-only} 모든 센서의 신호는 반드시 BSPD로 직접 전송되어야 한다. 다른 모듈의 출력은 센서 신호 대신 사용될 수 없다.
      ...
      \item\label{rule:formula-technical.electronic-throttle-control.bspd.trip-mimics-estop} 위 조건 중 하나라도 발생하면 제동시스템 타당성장치(BSPD)는 주 비상 정지 스위치(\ref{item:주 비상 정지 스위치})를 개방한 것과 동일하게 작동하여 다음과 같은 사항을 만족해야 한다.
        \begin{enumerate}
...
\begin{enumerate}
  ...
  \item\label{rule:formula-technical.components.apps} 가속 페달 위치 센서 (Accelerator Pedal Position Sensor - APPS)
    \begin{enumerate}
      ...
      \item\label{rule:formula-technical.components.apps.open-short-fault-range} 아날로그 신호를 사용할 경우 개방(Open) 또는 대비하여 동작 범위를 설정하여야 한다. (예를 들면, 5V 전압 사용시 동작 범위를 0.5V \string~ 4.5V로 설정하여 출력이 0.5V보다 작거나 4.5V보다 클 경우에는 동작신호 미출력(Failure)으로 인식하도록 설정해야 한다.)
...
\begin{enumerate}
  ...
  \item\label{rule:formula-technical.grounding.nearby-parts-max-100ohm} 구동시스템의 부품으로부터 100mm 이내에 전기 전도성 부품 (예를 들어, 완전히 코팅된 금속 부품, 탄소 섬유 부품 등)이 될 수 있는 차량 부품은 GLVS Ground와 100Ω 미만의 저항을 가져야 한다.
...
\begin{table}[H]
  ...
  \begin{tblr}{
    ...
    2019. 2.21 개정 & 2020. 1.15 개정 & 2020.12.17 개정 & 2022. 2.17 개정   \\
...
\begin{table}[H]
  ...
  {\footnotesize
    \begin{tblr}{
      ...
      \ref{item:모노코크 메인 롤 후프}    & 메인롤후프와 모노코크의 접합부 Main Roll Hoop Attachment to Monocoque   & & \\
      ...
      \ref{section:측면 충돌 보호 구조}   & 측면 충격 보호 구조 Side Impact Protection                          & & \\
